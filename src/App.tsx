...
export default function App() {
  ...
  useEffect(() => {
    if (appSettings) {
      ...
      const timestamp = appSettings.updated_at ? new Date(appSettings.updated_at).getTime() : Date.now();
      const cacheBust = `?v=${timestamp}`;
      
      const iconUrl = appSettings.app_icon_url || `/icon.svg${cacheBust}`;
      const pngIconUrl = appSettings.app_icon_url || `/icon.png${cacheBust}`;
      ...
      const splashUrl = appSettings.splash_screen_url || `/splash.png${cacheBust}`;
