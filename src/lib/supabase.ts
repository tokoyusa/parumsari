...
export const db = {
  ...
  async getAppSettings() {
    ...
    if (fetchedFromDb) {
      try {
        localStorage.setItem('umkm_settings', JSON.stringify(baseSettings));
      } catch (e) {
        console.error('Error caching settings to localStorage:', e);
      }
      return baseSettings;
    }

    // Fallback to local storage if offline or fetch failed
    try {
      const localStr = localStorage.getItem('umkm_settings');
      if (localStr) {
        return this.parseAppSettings(JSON.parse(localStr));
      }
    } catch (e) {
      console.error('Error reading local settings cache:', e);
    }
