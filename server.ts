      ...
      if (response.ok) {
        ...
        if (data && data.length > 0) {
          ...
          if (settings.updated_at) {
            // Get last 10 characters or hash/timestamp to use as version
            const timestamp = new Date(settings.updated_at).getTime();
            updatedAtStr = `?v=${timestamp}`;
          }
        }
      }
    } catch (e) {
...
app.get("/icon.png", async (req, res, next) => {
  ...
  const setCachingHeaders = (responseObj: any) => {
    if (req.query.v) {
      responseObj.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
    } else {
      responseObj.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, private');
    }
  };
  ...
  setCachingHeaders(res);
  res.redirect('/icon.svg' + (req.query.v ? `?v=${req.query.v}` : ''));
});
...
app.get("/splash.png", async (req, res, next) => {
  ...
  setCachingHeaders(res);
  res.redirect('/splash.svg' + (req.query.v ? `?v=${req.query.v}` : ''));
});
